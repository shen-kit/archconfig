...
local function factory(args)
	...
	for i = 1, #batteries do
		BatNow.n_status[i] = "N/A"
		BatNow.n_perc[i] = 0
		BatNow.n_capacity[i] = 0
	end
	...
	function bat.update()
		...
		for i, battery in ipairs(batteries) do
			...
			if tonumber(present) == 1 then
				...
				BatNow.n_status[i] = helpers.first_line(bstr .. "/status") or "N/A"
				BatNow.n_perc[i] = energy_percentage or BatNow.n_perc[i]

				if not charge_design or charge_design == 0 then
					BatNow.n_capacity[i] = 0
				else
					BatNow.n_capacity[i] = math.floor((charge_full / charge_design) * 100)
				end
		...
		BatNow.capacity = math.floor(math.min(100, (sum_charge_full / sum_charge_design) * 100))
		...
		BatNow.status = BatNow.n_status[1] or "N/A"
		for _, status in ipairs(BatNow.n_status) do
			if status == "Discharging" or status == "Charging" then
				BatNow.status = status
			end
		end
		BatNow.ac_status = tonumber(helpers.first_line(string.format("%s%s/online", pspath, ac))) or "N/A"

		if BatNow.status ~= "N/A" then
			if BatNow.status ~= "Full" and sum_rate_power == 0 and BatNow.ac_status == 1 then
				BatNow.perc = math.floor(math.min(100, (sum_energy_now / sum_energy_full) * 100))
				BatNow.time = "00:00"
				BatNow.watt = 0

			-- update {perc,time,watt} iff battery not full and rate > 0
			elseif BatNow.status ~= "Full" then
				local rate_time = 0
				...
				if sum_rate_power > 0 or sum_rate_current > 0 then
					...
					if 0 < rate_time and rate_time < 0.01 then -- check for magnitude discrepancies (#199)
						local rate_time_magnitude = math.abs(math.floor(math.log(rate_time, 10)))
				...
				BatNow.perc = math.floor(math.min(100, (sum_energy_now / sum_energy_full) * 100))
				BatNow.time = string.format("%02d:%02d", hours, minutes)
				BatNow.watt = tonumber(string.format("%.2f", sum_rate_energy / 1e6))
			elseif BatNow.status == "Full" then
				BatNow.perc = 100
				BatNow.time = "00:00"
				BatNow.watt = 0
		...
		if notify == "on" then
			if BatNow.status == "Discharging" then
				if tonumber(BatNow.perc) <= n_perc[1] then
					bat.id = naughty.notify({
						preset = bat_notification_critical_preset,
						replaces_id = bat.id,
					}).id
				elseif tonumber(BatNow.perc) <= n_perc[2] then
					bat.id = naughty.notify({
			...
			elseif BatNow.status == "Full" and full_notify == "on" and not fullnotification then
				bat.id = naughty.notify({
